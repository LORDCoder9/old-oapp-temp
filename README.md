...
3. Your private server name will be at <http://oapp.surge.sh/#host=oapp-server-template.glitch.me> (replace `oapp-server-template` with it's new name)
...
```
3. Open the private server with <http://oapp.surge.sh/#host=oapp-server-template.glitch.me&key=aSecretPasswordHere> (replace `aSecretPasswordHere` with the password)
...
You can add a gamemode code in the link to specify its gamemode, such as <http://oapp.surge.sh/host=oapp-server-template.glitch.me&mode=f> where `f` is the gamemode code.
...
- `o` for Open
- `m` for Maze
- `f` for FFA, `2` for 2 Teams, `3` for 3 Teams, `4` for 4 Teams, or `s` for Survival
